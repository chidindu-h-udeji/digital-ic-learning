...
module tb_if_id_register;
  ...
  initial begin
    ...
    // SCENARIO 3: Priority Conflict (Tested from a non-bubble state)
    reset = 1;
    stall = 1;
    flush = 1;
    ...
    #1;
    if (pc_out !== 0) begin
      $display("ERROR (Scen 3, pc_out): Expected reset output (0), got %h", pc_out);
      errors = errors + 1;
    end
    if (pc_plus4_out !== 0) begin
      $display("ERROR (Scen 3, pc_plus4_out): Expected reset output (0), got %h", pc_plus4_out);
      errors = errors + 1;
    end
    if (instruction_out !== 32'h00000013) begin
      $display("ERROR (Scen 3, instruction_out): Expected reset output 00000013, got %h", instruction_out);
      errors = errors + 1;
    end
    
    // RECOVERY: Re-establish the baseline non-bubble state for the stall test
    reset = 0; 
    stall = 0; 
    flush = 0;
    @(negedge clk);
    ...
    #1;
    if (pc_out !== 32'h91CE) begin
      $display("ERROR (Scen 4, pc_out): Expected 91CE, got %h", pc_out);
      errors = errors + 1;
    end
    if (pc_plus4_out !== 32'h512E) begin
      $display("ERROR (Scen 4, pc_plus4_out): Expected 512E, got %h", pc_plus4_out);
      errors = errors + 1;
    end
    if (instruction_out !== 32'hD1CE) begin
      $display("ERROR (Scen 4, instruction_out): Expected D1CE, got %h", instruction_out);
      errors = errors + 1;
    end
